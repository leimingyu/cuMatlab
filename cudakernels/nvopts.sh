...
#----------------------------------------------------------------------------
            ...
            CC='nvcc'
            CFLAGS='-O3 -Xcompiler "-fPIC -D_GNU_SOURCE -pthread -fexceptions -m32 -fno-strict-aliasing"'		
			#CFLAGS="$CFLAGS -gencode=arch=compute_10,code=\"sm_10,compute_10\" -gencode=arch=compute_20,code=\"sm_20,compute_20\""
			CFLAGS="$CFLAGS -gencode arch=compute_30,code=sm_30 "
			CFLAGS="$CFLAGS -gencode arch=compute_35,code=sm_35 "
			CFLAGS="$CFLAGS -gencode arch=compute_52,code=sm_52 "
			CFLAGS="$CFLAGS -gencode arch=compute_53,code=sm_53 "

		    #CT 7.2011 only for fermi architectures and 1.x, with no PTX JIT support for future versions
            CLIBS="-lstdc++ $RPATH $MLIBS -lm"
            COPTIMFLAGS='-Xcompiler "-O3 -msse2 -DNDEBUG"'
            CDEBUGFLAGS='-g'
#           
            CXX='g++'
            CXXFLAGS='-fPIC -D_GNU_SOURCE -pthread '
            CXXLIBS="-lstdc++ $RPATH $MLIBS -lm"
            CXXOPTIMFLAGS='-O3 -funroll-loops -msse2 -DNDEBUG'
            CXXDEBUGFLAGS='-g'
#
...
#----------------------------------------------------------------------------
            RPATH="-Wl,-rpath-link,$TMW_ROOT/bin/$Arch"
            CC='nvcc'
			XCOMPILERFLAGS='-fPIC -D_GNU_SOURCE -pthread -m64 -fexceptions -fno-strict-aliasing'
#           CFLAGS="$CFLAGS -m64 -O3 -gencode=arch=compute_13,code=sm_13 -gencode=arch=compute_20,code=sm_20"
            CFLAGS="$CFLAGS -m64 -O3"
            CFLAGS="$CFLAGS -gencode arch=compute_30,code=sm_30"
            CFLAGS="$CFLAGS -gencode arch=compute_35,code=sm_35"
            CFLAGS="$CFLAGS -gencode arch=compute_52,code=sm_52"
            CFLAGS="$CFLAGS -gencode arch=compute_53,code=sm_53"

			CFLAGS="$CFLAGS -Xcompiler \"$XCOMPILERFLAGS\""
            CLIBS="-lstdc++ $RPATH $MLIBS -lm"
			XCOMPILEROPTIMFLAGS='-O3 -msse2 -DNDEBUG'
            COPTIMFLAGS="-Xcompiler \"$XCOMPILEROPTIMFLAGS\""
            CDEBUGFLAGS='-g'
#           
            CXX='g++'
...
#
            LD="gcc"
            LDEXTENSION='.mexa64'
            LDFLAGS="$XCOMPILERFLAGS -shared -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE" # CT 7.2011 must include compiler flags see gcc manual on linker
            LDOPTIMFLAGS="$XCOMPILEROPTIMFLAGS"
            LDDEBUGFLAGS='-g'
#
            POSTLINK_CMDS=':'
#----------------------------------------------------------------------------
